...
Each OpenAI resource group has a protocol in `protocols.py`. Backend clients implement the protocols they support. At startup the gateway iterates `KNOWN_BACKENDS`, instantiates each client whose env var is set, and uses `isinstance` checks to wire `app.state` slots automatically.

To add a new backend: create a client class in `backends/`, extend `BaseBackend`, implement the relevant protocols, register it in `KNOWN_BACKENDS`.
